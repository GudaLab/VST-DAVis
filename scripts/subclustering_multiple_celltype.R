datainput_subclustering_multiple_celltype <- function(index_subclustering_multiple_celltype_input, index_cell_markers, index_m_subclustering_celltype1, index_m_subclustering_celltype2, index_m_subclustering_celltype3, index_m_subclustering_celltype4, index_m_subclustering_celltype5, index_m_subclustering_celltype6, index_m_subclustering_celltype7, index_m_subclustering_celltype8, index_m_subclustering_celltype9, index_m_subclustering_clustering6, index_subclustering_multiple_sample_normalization_method){
  ...
  index_m_subclustering_celltype8 <- isTRUE(as.logical(index_m_subclustering_celltype8))
  ...
  build_annotation_result <- function(obj, annotation_field, table_data, extra_plot1 = NULL, extra_plot2 = NULL) {
    ...
    dim_plot <- build_annotation_dimplot(
      ...
      reduction_name = index_m_subclustering_clustering6,
      label_points = index_m_subclustering_celltype8,
      requested_split = index_m_subclustering_celltype9
    )
  ...
  if (as.character(index_m_subclustering_celltype1) == "1") {
    ...
    tissue <- index_m_subclustering_celltype2
    gs_list <- gene_sets_prepare(db_, tissue)
    sctype_assay <- pick_annotation_assay(subclustering_multiple_sample_clustering, index_subclustering_multiple_sample_normalization_method)
    sctype_input <- get_sctype_input(subclustering_multiple_sample_clustering, assay_name = sctype_assay)
    ...
    cL_resutls <- do.call("rbind", lapply(unique(subclustering_multiple_sample_clustering@meta.data$seurat_clusters), function(cl) {
      es.max.cl <- sort(
        rowSums(es.max[, rownames(subclustering_multiple_sample_clustering@meta.data[subclustering_multiple_sample_clustering@meta.data$seurat_clusters == cl, ])]),
        decreasing = TRUE
      )
      head(
        data.frame(
          ...
          scores = es.max.cl,
          ncells = sum(subclustering_multiple_sample_clustering@meta.data$seurat_clusters == cl)
        ),
        10
      )
    }))
    ...
    subclustering_multiple_sample_clustering@meta.data$sctype_classification <- sanitize_annotation_values(
      subclustering_multiple_sample_clustering@meta.data$sctype_classification
    )

    return(build_annotation_result(
      obj = subclustering_multiple_sample_clustering,
      annotation_field = "sctype_classification",
      table_data = sctype_scores
    ))
  }

  if (as.character(index_m_subclustering_celltype1) == "2") {
    ref <- celldex::fetchReference(index_m_subclustering_celltype3, "2024-02-26")
    singleR_assay <- pick_annotation_assay(subclustering_multiple_sample_clustering, index_subclustering_multiple_sample_normalization_method)
    subclustering_multiple_sample_clustering.counts <- get_assay_matrix_with_fallback(
    ...
    Pred <- SingleR::SingleR(
      test = subclustering_multiple_sample_clustering.counts,
      ref = ref,
      labels = ref$label.main,
      assay.type.test = 1,
      de.method = index_m_subclustering_celltype4
    )
    Pred1 <- Pred %>% data.table::as.data.table()
    Pred1$cells <- rownames(Pred)
    subclustering_multiple_sample_clustering$singleR_labels <- sanitize_annotation_values(
      Pred$labels[match(rownames(subclustering_multiple_sample_clustering@meta.data), rownames(Pred))]
    )
  ...
  if (as.character(index_m_subclustering_celltype1) == "4") {
    cell_type <- index_m_subclustering_celltype7
    names(cell_type) <- levels(subclustering_multiple_sample_clustering)
    subclustering_multiple_sample_clustering@meta.data$cell_type <- sanitize_annotation_values(
      cell_type[as.character(Seurat::Idents(subclustering_multiple_sample_clustering))]
    )
    manual_table <- data.frame(
      cluster = levels(subclustering_multiple_sample_clustering),
  ...
  stop("Unsupported subclustering cell type annotation mode: ", index_m_subclustering_celltype1)
}
