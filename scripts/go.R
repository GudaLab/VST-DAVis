datainput_single_multiple_sample_go<- function(index_multiple_sample_go_input, index_subclustering_multiple_sample_go_input, index_multiple_sample_go_input2, index_subclustering_multiple_sample_go_input2, index_multiple_sample_go_input3, index_subclustering_multiple_sample_go_input3, index_s_go1, index_s_go2, index_s_go3, index_s_go4, index_s_go5, index_s_go6, index_s_go7, index_s_go8, index_s_go9, index_s_go10, index_s_go11, index_s_go12, index_s_go13, index_s_go14){
  ...
  make_barplot <- function(enrich_obj, show_n) {
    res <- as.data.frame(enrich_obj@result)
    if (nrow(res) == 0) {
      return(make_info_plot("No GO terms are available to draw a bar plot."))
    }
  ...
  make_cnetplot_fallback <- function(enrich_obj, show_n) {
    ...
    if (!nrow(edges)) {
      return(make_info_plot("No gene-term connections are available for the selected GO terms."))
    }
    ...
    if (!nrow(edges)) {
      return(make_info_plot("The selected GO terms did not contain enough gene connections to draw a network."))
    }
    ...
    ggplot2::ggplot() +
      ...
      ggplot2::scale_x_continuous(breaks = c(1, 2), labels = c("GO term", "Gene"), limits = c(0.8, 2.2)) +
      ggplot2::coord_cartesian(clip = "off") +
      ggplot2::theme_void() +
      ggplot2::theme(plot.margin = ggplot2::margin(10, 70, 10, 120)) +
      ggplot2::ggtitle("GO term-gene network")
  }

  make_upsetplot_fallback <- function(enrich_obj, show_n) {
    edges <- extract_term_gene_edges(enrich_obj, show_n)
    if (!nrow(edges)) {
      return(make_info_plot("No gene-set intersections are available for the selected GO terms."))
    }
    ...
    if (!length(combo_labels)) {
      return(make_info_plot("No overlapping GO term intersections were found."))
    }
    ...
    ggplot2::ggplot(combo_df, ggplot2::aes(x = Combination, y = Count)) +
      ...
      ggplot2::theme_bw() +
      ggplot2::labs(
        title = "GO term intersection summary",
        x = "Intersection",
        y = "Gene count"
      )
  }
  ...
  if (length(deg_genes) == 0) {
    return(list(plot1 = make_info_plot("No valid genes were available for GO enrichment."), data1 = data.frame()))
  }

  entrez_ids <- tryCatch(
    clusterProfiler::bitr(deg_genes, fromType = "SYMBOL", toType = "ENTREZID", OrgDb = index_s_go5),
    error = function(e) data.frame()
  )

  entrez_ids <- unique(entrez_ids[!is.na(entrez_ids$ENTREZID) & nzchar(entrez_ids$ENTREZID), , drop = FALSE])
  if (nrow(entrez_ids) == 0) {
    return(list(plot1 = make_info_plot("No genes could be mapped to Entrez IDs for GO enrichment."), data1 = data.frame()))
  }

  entrez_gene_list <- entrez_ids$ENTREZID

  go_enrichment <- clusterProfiler::enrichGO(gene = entrez_gene_list,
                                             OrgDb = index_s_go5,
                                             ont = index_s_go6,
  ...
  go_enrichment <- sanitize_enrich_result(go_enrichment)
  if (nrow(as.data.frame(go_enrichment@result)) == 0) {
    return(list(plot1 = make_info_plot("No GO terms passed the selected thresholds."), data1 = data.frame()))
  }

  plots201 <- plot_enrichment(go_enrichment, index_s_go12, index_s_go13)
